function avg(arr) {
  ...
  export function computeCorrupcaoPontos(c) {
    ...
    if (c.primeiraDescoberta) {
      ...
      if (c.primeiroComprador) {
        compradores[c.primeiroComprador] =
          (compradores[c.primeiroComprador] || 0) - 1;
      }
    }
    ...
    if (c.segundaDescoberta) {
      ...
      if (c.segundoComprador) {
        compradores[c.segundoComprador] =
          (compradores[c.segundoComprador] || 0) - 1;
      }
    }
  ...
  export function computeSabotagemPontos(s) {
    ...
    if (s.descoberto) {
      ...
      if (
        s.tipoAcao === "vazar" &&
        s.denunciasConsecutivas >= 1
      ) {
        demitido = true;
      }
  
      if (
        s.tipoAcao === "atrapalhar" &&
        s.denunciasConsecutivas >= 2
      ) {
        demitido = true;
      }
    }
  ...
  export function computeEmpresaScore(data, empresa) {
    ...
    if (data.corrupcao.empresaCorruptora === empresa) {
      ajuste += cPts.corruptor;
    }
  
    if (data.sabotagem.empresaSabotador === empresa) {
      ajuste += sPts.sabotador + sPts.area;
    }
